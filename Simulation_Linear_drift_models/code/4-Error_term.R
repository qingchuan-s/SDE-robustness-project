...
# rho error term a 0_01 x 5.csv; b error term a 0_01 x 5.csv
# rho error term a 0_01 x 10.csv; b error term a 0_01 x 10.csv
# rho error term a 0_02 x 10.csv; b error term a 0_02 x 10.csv
...
sdp_list <- c(0,0.005,0.01,0.015)

## "ode-lse","ode-mle","sde-mle","sde-lse"
rho_list <- matrix(NA, nrow = length(sdp_list)*n, ncol = 5)
b_list <- matrix(NA, nrow = length(sdp_list)*n, ncol = 5)


for (i in 1:n) {
  
  r_dynamic <-  rnorm(length(times),0,1)
  r_gamma <- rnorm(length(times),0,1)
  ...
  for (nh in 1:length(sdp_list)){
    ...
    for (l in 2:length(times)){
      ...
      x_ode[l] <- x_ode[l-1] -a0*(x_ode[l-1]-b0)*dt + 
        gamma*(x_ode[l-1]-b0)^2*dt
      y_ode[l] <- x_ode[l] + sigma0 * r_dynamic[l]
      y_ou[l] <- y_ou[l-1] -a0*(y_ou[l-1]-b0)*dt +  
        gamma*(y_ou[l-1]-b0)^2*dt + sigma*sqrt(dt)*r_dynamic[l]
    ...
    rho_list[(nh-1)*n+i,5]<- paste("sigma_gamma =", sdp)
    b_list[(nh-1)*n+i,5]<- paste("sigma_gamma =", sdp)
  }
}
...
#write.csv(df_rho1, "rho error term a 0_01 x 5 original.csv", row.names=FALSE)
#write.csv(df_b1, "b error term a 0_01 x 5 original.csv", row.names=FALSE)

#df_rho1 <- read.csv("rho error term a 0_01 x 5 original.csv")[,-1]
#df_b1 <- read.csv("b error term a 0_01 x 5 original.csv")[,-1]
...
#write.csv(df_rho, "rho error term a 0_01 x 5.csv", row.names=FALSE)
#write.csv(df_rho, "rho error term a 0_01 x 10.csv", row.names=FALSE)
#write.csv(df_rho, "rho error term a 0_02 x 10.csv", row.names=FALSE)
...
#write.csv(df_b, "b error term a 0_01 x 5.csv", row.names=FALSE)
#write.csv(df_b, "b error term a 0_01 x 10.csv", row.names=FALSE)
#write.csv(df_b, "b error term a 0_02 x 10.csv", row.names=FALSE)
...
df_rho1 <- read.csv("rho error term a 0_01 x 5.csv")
df_b1 <- read.csv("b error term a 0_01 x 5.csv")

df_rho1 <- read.csv("rho error term a 0_01 x 10.csv")
df_b1 <- read.csv("b error term a 0_01 x 10.csv")

df_rho1 <- read.csv("rho error term a 0_02 x 10.csv")
df_b1 <- read.csv("b error term a 0_02 x 10.csv")
...
p1 <- ggplot(data = df_rho1_plot ,aes(x = value, color=Perturbation))+
  ...
  scale_color_manual(labels = c(TeX("$\\sigma_\\gamma = 0.005$  "), TeX("$\\sigma_\\gamma = 0.01$  "),TeX("$\\sigma_\\gamma = 0.015$  ")),values = c(3,6,4))+
...
p1<- p1 + 
  scale_x_continuous( #limits   = c(0.975, 1.025), # for 0.01 * 10
    limits   = c(0.98, 1.02), # for 0.02 * 10
    sec.axis = sec_axis(~ . , name = "Data-generating model", breaks = NULL, labels = NULL))
p2<- p2+ 
  scale_y_continuous(sec.axis = sec_axis(~ . , name = "Fitted model", breaks = NULL, labels = NULL)) +
  scale_x_continuous(limits   = c(-2, 1), # for 0.01*5 and 0.02 * 10
                     #limits   = c(-2.5, 2.5), # for 0.01 * 10
    sec.axis = sec_axis(~ . , name = "Data-generating model", breaks = NULL, labels = NULL))
